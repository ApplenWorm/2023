...
\subsection{Real and polar representations}
...
\begin{soln}
    \begin{enumerate}[label=\alph*)]
        \item Denote $z = x + iy$. Then $z^2 = (x + iy)^2 = (x^2 - y^2) + i2xy \implies \text{Re } z^2 = x^2 - y^2, \text{Im } z^2 = 2xy$, and $f(x, y) = (x^2 - y^2, 2xy)$.
        ...
        \item $\vert z\vert  = \vert x + iy\vert  = \sqrt{x^2 + y^2} \implies f(x, y) = (\sqrt{x^2 + y^2}, 0) = \sqrt{x^2 + y^2} \to$ real function!
        \item $z = x + iy, \text{Re } z = x \implies f(x, y) = (x, 0) = x \to$ real function!
        $z = x + iy, \text{Im } z = y \implies$ for $f(z) = \text{Im } z: f(x, y) = (y, 0) = y \to$ real!
        \item Let $z = r(\cos \theta + i\sin \theta)$ \{$r = \vert z\vert $\}. Then, by De Moivre formula $z^n = r^n(\cos(n\theta) + i\sin(n\theta))$, and $f(r, \theta) = (r^n \cos(n\theta), r^n \sin(n\theta))$.
        \item Let $z = x + iy$. Then, by Euler's formula: $e^z = e^x(\cos y + i\sin y) \implies f(x, y) = (e^x \cos y, e^x \sin y)$.
    \end{enumerate}
\end{soln}
\end{example}
...
\begin{soln}
    \begin{enumerate}[label=\alph*)]
        ...
        \end{align*}
        \item $x^2 - 1 = \frac{1}{4}(z^2 + \bar{z}^2 + 2z\bar{z}) - 1 = \frac{1}{4}(z^2 + \bar{z}^2 + 2z\bar{z} - 4)$.
    \end{enumerate}
\end{soln}
...
\includegraphics{image_lin7.pdf}
...
\includegraphics{image_lin8.pdf}
...
\item Show that $w = \frac{1}{z}$ maps the interior of the unit circle in the first quadrant to the exterior of the unit circle in the fourth quadrant.

\includegraphics{image_lin9.pdf}

\item What is the image of the unit circle under $\frac{1}{z}$?
\item Write down how $z \to \frac{1}{z} = w$ maps the punctured plane $\mathbb{C} \setminus \{0\}$ to itself (and $\mathbb{C} \cup \{\infty\}$ to itself).
...
\begin{example}
Back to Ex~\ref{example:4.5}. Let us study two images under $\frac{1}{z}$.
    \begin{enumerate}[label=\alph*)]
        \item Find the image of the line $z = t + i(1 - t)$ under $\frac{1}{z}$.
        \item Determine how the horizontal lane $y = k$ is transformed under $w = \frac{1}{z}$.
    \end{enumerate}

\begin{soln}
    \begin{enumerate}[label=\alph*)]
        \item Denote: $w = u + iv, z = x + iy \implies x = t, y = (1 - t)$.
...
Namely, the map $w = \frac{1}{z}$ maps the line $z = t + i(1 - t)$ to the circle (in $w$-plane) centered at the point $(\frac{1}{2}, -\frac{1}{2})$ with radius $\frac{1}{\sqrt{2}}$.
        ...
        \includegraphics{image_lin10.pdf}
        \item Substitute as before: $x = \frac{u}{u^2 + v^2}, y = \frac{-v}{u^2 + v^2}$. Plug $y = k$:
        \begin{align*}  
...
\includegraphics{image_lin11.pdf}

\includegraphics{image_lin12.pdf}
...
\subsection{Definition and properties of limits}
...
\begin{example}
Let $f(z) = \frac{\bar{z}}{2}$. Claim: for any $z_0$, $\lim_{z \to z_0} \frac{\bar{z}}{2} = \frac{\bar{z_0}}{2}$.

\begin{proof}
    Given $\epsilon > 0$, choose $\delta = 2\epsilon$
    %  \vert z_0\vert $
     . Then, if $0 < \vert z - z_0\vert  < \delta$:
    \begin{align*}
    \left\vert \frac{\bar{z}}{2} - \frac{\bar{z_0}}{2}\right\vert  & = \left\vert \frac{\bar{z} - \bar{z_0}}{2}\right\vert  = \frac{\vert \bar{z} - \bar{z_0}\vert }{2} = \frac{\vert \overline{z - z_0}\vert }{2} = \frac{\vert z - z_0\vert }{2} < \frac{\delta}{2} = \frac{2\epsilon}{2} = \epsilon 
    \end{align*}
    Thus, for a given $\epsilon > 0$ we have found $\delta$ (that depends on $\epsilon$!) s.t. if $\vert z - z_0\vert  < \delta$, then $\vert f(z) - w_0\vert  = \left\vert \frac{\bar{z}}{2} - \frac{\bar{z_0}}{2}\right\vert  < \epsilon$ (for any $z_0$!).
\end{proof}
\end{example}
...
% & \leq \frac{\vert \bar{z}\vert \vert z_0 - z\vert  + \vert z\vert \vert \bar{z_0} - \bar{z}\vert }{\vert \bar{z}\vert \vert \bar{z_0}\vert } = \frac{\vert z_0 - z\vert }{\vert z_0\vert } + \frac{\vert z\vert }{\vert z\vert } \frac{\vert z - z_0\vert }{\vert z_0\vert } \\
% &= \frac{2\vert z - z_0\vert }{\vert z_0\vert } < \frac{2\delta}{\vert z_0\vert } = \frac{2 \cdot 2\epsilon\vert z_0\vert }{\vert z_0\vert } = \epsilon \\
% \end{align*}
...
\begin{proof}
 Assume $\lim_{z \to z_0} f(z) = a$ and $\lim_{z \to z_0} f(z) = b$. By Def~\ref{definition:3.1} for any $\epsilon > 0$ there exist $\delta_1 > 0$ and $\delta_2 > 0$ such that
\begin{align*}
...
\end{proof}
\end{claim}
Back to Ex~\ref{example:5.2}: we will use a common technique to show that the limit does not exist - we will show that there exist two different paths (directions) $z \to z_0$ along which we get 2 different limits.
...
\begin{example}
Ex~\ref{example:5.3} shows that the different paths need not be the real and imaginary axes!
\end{example}
...
By Def~\ref{definition:3.1}: given $\epsilon > 0$ there exists $\delta = \delta(\epsilon) > 0$ s.t. for all $z$ with $0 < \vert z - z_0\vert  < \delta$ we have $\vert f(z) - (u_0 + iv_0)\vert  < \epsilon$.

Since for any $z \in \mathbb{C}: \vert \text{Im } z\vert , \vert \text{Re } z\vert  \leq \vert \text{Re } z + i\text{Im } z\vert  = \vert z\vert $, we get:
\begin{align*}
 \vert v(x, y) - v_0\vert , \vert u(x, y) - u_0\vert  & < \vert u(x, y) - u_0 + i(v(x, y) - v_0)\vert  = \vert u(x, y) + iv(x, y) - (u_0 + iv_0)\vert  \\
 & = \vert f(z) - (u_0 + iv_0)\vert  < \epsilon 
 \end{align*}
