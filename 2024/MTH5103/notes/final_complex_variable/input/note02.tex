...
\end{align*}
\end{example}
\begin{example} 3.3 $f(z) = \sqrt{z - 1} + \sqrt{z + 1}$ is 4-valued: for example, if $z = 0$, we get $w_1 = i + 1, w_2 = -i + 1, w_3 = i - 1, w_4 = -i - 1$ (\verifythis)
\end{example}
...
\begin{remark} 
    If $\lim_{z \to z_0} f(z) = w_0$ exists, then $f(z) \to w_0$ as $z \to z_0$ on *every* path (uniqueness of the limit!) If on different paths $z \to z_0$ there are different values of the limit (or there exists a path on which the limit does not exist), then the limit $\lim_{z \to z_0} f(z)$ does not exist.
\end{remark}
