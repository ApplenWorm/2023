...
\begin{definition} Let $S \subseteq \mathbb{C}$ be a subset of complex numbers. A function $f$ on $S$ is a rule which assigns to each $z \in S$ a complex number (or complex numbers) $f(z)$, called the value of $f$ at $z$. $S$ is called the Domain of Definition (DoD).
\end{definition}
\begin{definition} If for every $z$ in the DoD corresponds one value $w$, then we say that $f$ is a single-valued function. If to $z$ correspond several values, then $f$ is a multi-valued function.
\end{definition}
\begin{example} 3.1 $f(z) = \frac{1}{z}$: $f$ is not defined at $z = 0$ and it is defined and single-valued everywhere else.
\end{example}
...
\begin{example} 3.2 $f(z) = \sqrt{z + 1}$ defined everywhere on $\mathbb{C}$ and it is double-valued function: $f(0) = 1$ and $f(0) = -1$ (for instance).
...
\end{example}
\begin{example} 3.3 $f(z) = \sqrt{z - 1} + \sqrt{z + 1}$ is 4-valued: for example, if $z = 0$, we get $w_1 = i + 1, w_2 = -i + 1, w_3 = i - 1, w_4 = -i - 1$ (CHECK!)
\end{example}
\begin{example} 3.4 Polynomials. General form: $P_n(z) = \sum_{j=0}^{n} a_j z^j$ - polynomial of degree $n$ of complex variable $z$ with complex coefficients $a_j \in \mathbb{C}$.
\end{example}
For example: $f(z) = z^2, f(z) = 4 + iz - z^2 + (1 + i)z^3$.

\begin{example} 3.5 Rational function \{ratio of two polynomials\}. General form:
...
\end{example}
\begin{example} 3.6 The exponential function $f(z) = e^z$.
...
\begin{example} 3.7 Trigonometric and Hyperbolic functions. For $z \in \mathbb{C}$ define:
...
\begin{example} 3.8 Write in the form (1) or (3)
...
\begin{example} 3.9 Rewrite in terms $z$ and $\bar{z}$:
...
\begin{example} 4.1 Let $f(z) = \text{Im } z$ (defined and single-valued everywhere). Let us find its image \{namely, where to the complex plane $\mathbb{C}$ is mapped\}.
$\text{Im } z = y \in \mathbb{R} \implies$ the complex plane $\mathbb{C}$ is mapped to $\mathbb{R}$.
\end{example}
\begin{example} 4.2 What is the image of the ray $\arg z = \frac{\pi}{3}$ under $f(z) = z^3$?
...
\begin{example} 4.4 Determine which regions (if any) of the z-plane are mapped to the upper half part of the w-plane under the mapping $w = z^2$.
...
\begin{definition}  $\lim_{z \to z_0} f(z) = w_0$ if for any given $\epsilon > 0$, there exists $\delta = \delta(\epsilon) > 0$ such that if $0 < \vert z - z_0\vert  < \delta$, then $\vert f(z) - w_0\vert  < \epsilon$.
\end{definition}
...
\end{soln}
Note: 
\begin{remark} 8989 For any $z_0 \neq 0$, $\lim_{z \to z_0} \frac{z}{\bar{z}} = \frac{z_0}{\bar{z_0}}$. Pf:
...
\begin{example} 5.3 Show that $\lim_{z \to 0} \frac{\bar{z}^2}{z^2}$ does not exist.
\begin{soln} Here, for $z = x \in \mathbb{R}:  \frac{\bar{z}^2}{z^2} = \frac{x^2}{x^2} = 1$ and for $z=iy \in i\mathbb{R}: \frac{\bar{z}^2}{z^2} = \frac{(-iy)^2}{(iy)^2} = \frac{-y^2}{-y^2}=1$. However, choosing the path $z=(1+i)t$ for $t \in \mathbb{R} \setminus \{0\}$ gives us $\frac{\bar{z}^2}{z^2}=\frac{((1-i)t)^2}{((1+i)t)^2} = \frac{(1-i)^2}{(1+i)^2} = \frac{1-2i-1}{1+2i-1}=-1$. $-1 \neq 1$ - does not agree with the value on previous two paths, thus we can conclude that the original limit does not exist.
\end{soln}
\end{example}
...
\begin{proposition} 3.1 If $\lim_{z \to z_0} f(z) = w_0$ and $\lim_{z \to z_0} g(z) = w_1$, then
\begin{enumerate}
...
Pf: \begin{proof} EXERCISE - a very good exercise to "play" with $\epsilon$-$\delta$ definition! \end{proof}
\end{proposition}
...
\begin{proposition} 3.2 Let $f(z) = u(x, y) + iv(x, y), z_0 = x_0 + iy_0$. Then $\lim_{z \to z_0} f(z) = u_0 + iv_0 = w_0 \iff \lim_{(x, y) \to (x_0, y_0)} u(x, y) = u_0 = \text{Re } w_0$ and $\lim_{(x, y) \to (x_0, y_0)} v(x, y) = v_0 = \text{Im } w_0$.
\end{proposition}
Note: the first limit is complex, but the last two limits are real!

\begin{proof} Pf: $\implies$ Assume that $\lim_{z \to z_0} f(z) = u_0 + iv_0$. Let us show that $\lim_{(x, y) \to (x_0, y_0)} u(x, y) = u_0 = \text{Re } w_0$ and $\lim_{(x, y) \to (x_0, y_0)} v(x, y) = v_0 = \text{Im } w_0$.
