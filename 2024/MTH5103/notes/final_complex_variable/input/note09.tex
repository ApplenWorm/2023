...
\textbf{Ex 19.11:} Compute $\int_C f(z) dz$ where $f(z) = \bar{z}$ and C connects 2 points $z_0 = 0, z_1 = 2 + 4i$
...
\textbf{Ex 19.12:} Let $C_1$ be the contour from -1 to 1 along the upper unit semicircle and $C_2$ the contour from -1 to 1 along the 
...
\textbf{Ex 19.13:} Compute $\int_C \frac{dz}{(z-z_0)^n}$, where $C = \{z \in \mathbb{C}: |z-z_0| = R\}$ and $n$ is some integer.
...
\textbf{Ex 19.4:} Compute $\int_C \frac{dz}{\sqrt{z}}$ where $C = \{f(x,y): x^2 + y^2 = 16, y \geq 0\}$, and the branch of $\sqrt{z}$ is such that $\sqrt{4} = -2$.
...
\textbf{Ex 20.1:} Compute $\int_C \frac{z}{(z-4)^2} dz$, where C is the contour defined by traversing once the square with vertices $\pm 3i, 6 \pm 3i$ anticlockwise.
