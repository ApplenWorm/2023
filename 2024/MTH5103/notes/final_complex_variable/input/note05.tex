...
\begin{proof}
    ...
    $\implies \lim_{N \to \infty} z_N = \lim_{N \to \infty} (S_N - S_{N-1}) = \lim_{N \to \infty} S_N - \lim_{N \to \infty} S_{N-1} = S - S = 0$ by Prop~\zref{proposition:10.1}. $s$ is unique.
\end{proof}
\end{proposition}
...
\begin{remark}
The converse of Prop~\ref{proposition:11.2} is false: $\lim_{n \to \infty} \frac{1}{n} = 0$, but the series $\sum_{n=1}^{\infty} \frac{1}{n}$ diverges!
\end{remark}
...
\begin{proof}
    \begin{enumerate}
        \item follows directly from Def~\ref{definition:10.1} and the application of triangle inequality: $|x_n - x|, |y_n - y| \leq |z_n - z| \leq |x_n - x| + |y_n - y|$
...
\begin{proof}
    ...
    Thus, by Prop~\ref{proposition:11.3} 2) $\sum_{n=0}^{\infty} z_n$ converges.
\end{proof}
\end{proposition}
...
A power series is an example of complex series where the sequence $z_n$ from Def~\ref{definition:11.1} is of the special form $\{a_n(z-z_0)^n\}$ - involves variable $z$. 
...
\begin{remark}
From Def~\ref{definition:12.1} every power series $\sum_{n=0}^{\infty} a_n(z-z_0)^n$ converges for at least one point $z = z_0$.
\end{remark}
...
\end{theorem}
    \begin{proof}
        Since $\sum_{n=0}^{\infty} a_n z_0^n$ converges by (Prop~\ref{proposition:11.1}) $\lim_{n \to \infty} a_n z_0^n = 0$, 
...
Note: From Prop~\ref{proposition:12.2}: $f(z) = \sum_{n=0}^{\infty} a_n (z - z_0)^n$ is differentiable arbitrarily many times at any $z$ in the disc of convergence 
...
\begin{proof}
    By Prop~\ref{proposition:12.2} 1) $e^z$ is analytic for every $z \in \mathbb{C} \implies$ entire (since $R = \infty$). By Prop~\ref{proposition:12.2} 2): $(e^z)' = \sum_{n=1}^{\infty} \frac{nz^{n-1}}{n!} = \sum_{n=1}^{\infty} \frac{z^{n-1}}{(n-1)!} = \sum_{n=0}^{\infty} \frac{z^n}{n!} = e^z$.
\end{proof}
\end{proposition}
...
\begin{proof}
    ...
    By Def~\ref{definition:12.4}: $e^{z_1} = \sum_{n=0}^{\infty} \frac{z_1^n}{n!}, \quad e^{z_2} = \sum_{n=0}^{\infty} \frac{z_2^n}{n!}$.
