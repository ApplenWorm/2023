...
\begin{example}\label{example:12.3} Let $z_n = (i)^n: z_0 = 1, z_1 = i, z_2 = -1, z_3 = -i, z_4 = 1, z_5 = i, \dots$. This is a bounded sequence ($|z_n| \leq 1$ for any $n$), but $\lim_{n \to \infty} z_n$ does not exist: there is no $z$ such that almost all but finitely many points of the sequence are in a small neighborhood of some $z$: $z_n$ jumps at every step.
\end{example}
...
\begin{definition}\label{definition:11.1} The complex (scalar) series $\sum_{n=1}^{\infty} z_n$ converges to the value $s \in \mathbb{C}$ if the sequence of partial sums $\{S_N\}_{N=0}^{\infty}, S_N = z_0 + z_1 + \dots + z_N$ converges to $s$. The value $s$ is called the sum of the series. If the limit does not exist (or $\infty$) ($\lim_{N \to \infty} S_N$) then the series diverges. If the series converges to some value $s$, then $s$ is unique.
\end{definition}
...
\begin{proof}
    By Def 11.1: If $\sum_{n=0}^{\infty} z_n = S$, then $\lim_{N \to \infty} S_N = \lim_{N \to \infty} \sum_{n=0}^{N} z_n = S$. On the other hand: $S_N - S_{N-1} = (z_0 + z_1 + \dots + z_N) - (z_0 + z_1 + \dots + z_{N-1}) = z_N$
...
\begin{proposition}\label{proposition:11.2} (Test for Divergence) Let $\{z_n\}_{n=0}^{\infty} \in \mathbb{C}$. If $\lim_{n \to \infty} z_n \neq 0$ (or does not exist), then the series $\sum_{n=0}^{\infty} z_n$ diverges.
\end{proposition}

\begin{remark}
The converse of Prop 11.2 is false: $\lim_{n \to \infty} \frac{1}{n} = 0$, but the series $\sum_{n=1}^{\infty} \frac{1}{n}$ diverges!
\end{remark}
...
\end{enumerate}
\begin{proof}
 1) follows directly from Def 10.1 and the application of triangle inequality: $|x_n - x|, |y_n - y| \leq |z_n - z| \leq |x_n - x| + |y_n - y|$
...
\begin{definition}\label{definition:12.1} The series $\sum_{n=0}^{\infty} a_n(z-z_0)^n, a_n \in \mathbb{C}, n \in \mathbb{N}$, is called a power series centered at $z = z_0$ (about $z = z_0$).
\end{definition}

A power series is an example of complex series where the sequence $z_n$ from Def 11.1 is of the special form $\{a_n(z-z_0)^n\}$ - involves variable $z$. 
...
\begin{remark}
From Def 12.1 every power series $\sum_{n=0}^{\infty} a_n(z-z_0)^n$ converges for at least one point $z = z_0$.
\end{remark}
...
\begin{theorem}{Abel}\label{theorem12.1}
    ...
    \begin{proof}
        Since $\sum_{n=0}^{\infty} a_n z_0^n$ converges by Prop 11.1 $\lim_{n \to \infty} a_n z_0^n = 0$, 
...
\begin{proposition}\label{definition:12.1} Given a power series $\sum_{n=0}^{\infty} a_n (z - z_0)^n$ there exists $0 \leq R \leq \infty$ \{a real non-negative number or else infinity\} such that if $|z - z_0| < R$, then $\sum_{n=0}^{\infty} a_n (z - z_0)^n$ converges absolutely and if $|z - z_0| > R$, then $\sum_{n=0}^{\infty} a_n (z - z_0)^n$ diverges.
\end{proposition}
...
\begin{example}\label{example:14.2} 1) The geometric series $\sum_{n=0}^{\infty} z^n$ - the radius of convergence $R = 1$ and the series diverges at any point on the circle $|z| = 1$ (by the Test for Divergence).
...
\begin{definition}\label{definition:12.3} The disc of convergence of the power series $\sum_{n=0}^{\infty} a_n (z - z_0)^n$ is given by $D = \{z \in \mathbb{C}: |z - z_0| < R\}$ and we write $f(z)$ for the sum of the series on this disc.
\end{definition}
...
\begin{proof}
    By Prop 12.2 1) $e^z$ is analytic for every $z \in \mathbb{C} \implies$ entire (since $R = \infty$). By Prop 12.2 2): $(e^z)' = \sum_{n=1}^{\infty} \frac{nz^{n-1}}{n!} = \sum_{n=1}^{\infty} \frac{z^{n-1}}{(n-1)!} = \sum_{n=0}^{\infty} \frac{z^n}{n!} = e^z$.
\end{proof}
\end{proposition}
...
\begin{proof}
    ...
    By Def 12.4: $e^{z_1} = \sum_{n=0}^{\infty} \frac{z_1^n}{n!}, \quad e^{z_2} = \sum_{n=0}^{\infty} \frac{z_2^n}{n!}$.
