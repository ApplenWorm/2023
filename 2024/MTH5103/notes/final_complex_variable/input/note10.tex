...
\begin{soln}
    ...
    Sol: Note: $f(z) = \frac{z}{(9-z^2)(z+i)}$ has simple poles at $z = -i, \pm 3$ (\verifythis) and only $z = -i$ is enclosed by $C$. Therefore:
    \begin{equation*}
...
Therefore, by the generalized Deformation Principle (Cor 18.6):
...
We shall sketch the proof of the case where $z_1, \dots, z_n$ are simple poles or removable singularities.
    ...
    \[
        ...
        If $z_k$ is a simple pole, then by Prop 16.2 ($m=1$) $f(z) = \frac{\varphi(z)}{z-z_k}$ where $\varphi(z)$ is analytic at $z_k$, therefore by CIF ( 18.7) \ref{theorem:Cauchy.Integral.formula.18.7}
        \[
...
Sol: (\verifythis) $z=0$ is a pole of order 2.
\[
...
Sol: $f(z) = \frac{1}{(z-1)(z-2)}$ has 2 simple poles $z=1, z=2$ (\verifythis !)
...
Sol: In the disc $\{z \in \mathbb{C}: |z| \le 3\}$ the function $f(z) = \frac{z+1}{(z-1)(z+2)^2}$ has 2 isolated singularities - poles at $z=1$ and at $z=-2$ (\verifythis). Therefore, by the Residue Theorem
\[
...
\begin{proof}
     ...
     Parametrize the contour $C$ by: $\gamma(\theta) = z_0 + Re^{i\theta}$, $0 \le \theta \le 2\pi$. By the CIF and the definition of the contour integral (Def 17.7)
        \[
