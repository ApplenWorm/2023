...
\graphicspath{{./input/graphs/note10/}}
...
We have proved Cauchy's theorem and have seen an important corollary of it - the Deformation Principle. 
We are about to see more important consequences of Cauchy's Theorem.
\begin{theorem}[The Cauchy Integral Formula]\label{theorem:Cauchy.Integral.formula.18.7}
    ...
    \begin{equation*}
        \textbf{CIF: } f(z_0) = \frac{1}{2\pi i} \oint_C \frac{f(z)}{z-z_0} dz
    \end{equation*}
    
    In other words: once we know the value of $f$ at every point on $C$, we can compute its value at every point inside $C$.
\end{theorem}
\begin{proof}
     Let $\epsilon > 0$. By Prop (\ref{proposition:6.5}) $f$ is continuous $\Rightarrow$ 
     there exists $\delta > 0$ such that if $|z-z_0| < \delta$, then $|f(z)-f(z_0)| < \epsilon$. 
     Choose $R > 0$ such that $R < \delta$ and such that the disc of radius $R$ centered at $z_0$ is inside $C$. 
     Let $C_0$ be the positively oriented boundary of that disc, parametrized by $\gamma(t) = z_0 + Re^{it}$, $0 \leq t \leq 2\pi$. 
     We have:
...
\end{proof}
\begin{example}\label{example:20.2}
     Compute $\oint_C \frac{z}{(9-z^2)(z+i)} dz$, where $C$ is the positively oriented circle centered at the origin of radius 2.
\end{example}
\begin{soln}

 \emphi{Note:} $f(z) = \frac{z}{(9-z^2)(z+i)}$ has simple poles at $z = -i, \pm 3$ (\verifythis) and only $z = -i$ is enclosed by $C$. Therefore:
    \begin{equation*}
	...
	By CIF (18.7)(\ref{theorem:Cauchy.Integral.formula.18.7}:
    %%%%%%
    $\int_C \frac{f(z)}{z+i} dz = 2\pi i f(-i) = 2\pi i \frac{-i}{-i-2} = 2\pi i \frac{-i}{-3} = \frac{\pi}{5}$.
\end{soln}

As this example suggests, there is a connection between the singularities of a given complex 
function $f(z)$ and the value of the contour integral for a given contour. In fact, there is an even 
stronger statement that one can make, relating the integral to the Laurent series expansion of $f$. 
We will not see it here.

\begin{example} \label{example:20.3} Compute $\int_C \frac{z^2 + 1}{z^2 + 2z - 3} dz$, where
    \begin{enumerate}
        \item $C_1 = \{z \in \mathbb{C}: |z| = 2\}$
        \item $C_2 = \{z \in \mathbb{C}: |z + 2 + 3i| = 4\}$
        \item $C_3 = \{z \in \mathbb{C}: |z + 1| = 1\}$
        \item $C_4 = \{z \in \mathbb{C}: |z + 1 + i| = 5\}$
    \end{enumerate}
\end{example}
\begin{soln}
    
     First: $z^2 + 2z - 3 = 0 \iff z = 1, z = -3$.
	...
	\includegraphics{img_graph5.pdf}
        ...
        \item 
        Since $|1 + i| = \sqrt{5} < 5, |-3 + i| = \sqrt{10} < 5$ both roots of the denominator lie in a domain bounded by $C_4$, therefore we cannot use CIF!
    \end{enumerate}
\end{soln}
%%%%
...
Then $\tilde{C_1} \cap \tilde{C_2} = \emptyset$ and $\tilde{C_1}, \tilde{C_2}$ lie interior to $C_4$. Then $f(z)$ is analytic in a domain that contains $C_4, \tilde{C_1}, \tilde{C_2}$, and the region between them. 
Therefore, by the generalized Deformation Principle (Cor (\ref{corollary:18.6})):
...
\begin{proof}
    ...
    (Sketch!): Choose a small, positively oriented circle around $z_0$ such that $f$ is analytic on and everywhere inside it. 
...
\emphi{Note:} one cannot always differentiate under an integral in this manner. 
We should, in fact, prove a technical lemma, justifying why we are allowed to perform such an operation. 
For example, the first derivative - we need to interchange $\lim_{\Delta z \to 0}$ and the integral and this requires justification!
\end{proof}
...
\begin{corollary}[Extended version of the CIF]\label{corollary:18.9}
    \emphi{Note:} Look at actual CIF (\ref{theorem:Cauchy.Integral.formula.18.7})
    (Extended version of the CIF): Let $f$ be analytic on and inside a simple, closed, positively oriented contour $C$ and let $z_0$ be inside $C$. Then, for all $n \ge 0$,
    \[
        f^{(n)}(z_0) = \frac{n!}{2\pi i} \int_C \frac{f(z)}{(z-z_0)^{n+1}} dz
        \]
    \end{corollary}

    \begin{example}\label{example:20.4}
        Compute $\int_C \frac{z^3}{(z-1)^4} dz$, where $C$ is any simple, closed, positively oriented contour around $z=1$.
    \end{example}

    \begin{soln}
      Set $f(z) = z^3$, apply Cor 18.9(\ref{corollary:18.9}) with $n=3$:
        \[
...
\begin{example}\label{example:20.5}
...
Recall (Def 16.2(\ref{definition:16.2})): The coefficient $b_1$ of the term $\frac{1}{z-z_0}$ is called the residue of the singularity at $z=z_0$, denoted by $\text{Res}(f, z_0)$. We have seen the definition and examples on week 8!

\section{Residue Theorem}

\subsection{Residues and singularities}
Now we are going to see how to compute integrals using only the values of the function at singularities - the residues.
\begin{theorem}[Residue Theorem]\label{theorem:residue.19.1}
...
\end{theorem}
\begin{proof}
We shall sketch the proof of the case where $z_1, \dots, z_n$ are simple poles or removable singularities.
...
Draw small positively oriented circles $C_k$ around the singularities $z_k$ such that they all inside $C$ and they are disjoint - choose radius small enough, so it is possible.
    ...
    By extended Deformation Principle (Cor 18.6(\ref{corollary:18.6})):
    \[
        ...
        If $z_k$ is a simple pole, then by Prop (\ref{proposition:16.2}) ($m=1$) $f(z) = \frac{\varphi(z)}{z-z_k}$ where $\varphi(z)$ is analytic at $z_k$, therefore by CIF ( 18.7) \ref{theorem:Cauchy.Integral.formula.18.7}
        \[
            \int_{C_k} f(z) dz = \int_{C_k} \frac{\varphi(z)}{z-z_k} dz = 2\pi i \varphi(z_k)
            \]
            
            But by Prop (\ref{proposition:16.2}) (for $m=1$) $\text{Res}(f, z_k) = \frac{\varphi(z_k)}{0!} = \varphi(z_k)$.
            
        \end{proof}
Now we can work out examples applying the Residue Theorem(\ref{theorem:residue.19.1}).
\begin{example}\label{example:21.1}
...
\begin{soln}
   Consider the Laurent series:
    ...
    \emphi{Note:} the singularity is $z=0$ that is a pole of order 4, and it is inside the disc $\{z \in \mathbb{C}: |z| \le 1\}$. Thus, the Residue Theorem is applicable.
...
%(165)
\begin{example}\label{example:21.2}Compute $\text{Res}(f, z_0)$ for $f(z) = \frac{3+4z}{(z+2)(z-i)}$ at $i, -2$.
\end{example}

\begin{soln}
    
    $z_0 = i$ is a simple pole since $\frac{3+4z}{z+2}|_{z=i} = \frac{3+4i}{i+2} \neq 0$ (and it is analytic at a neighborhood of $i$ and at $i$). In the same way (\verifythis) $z_0 = -2$ is a simple pole. Thus, from
    \[
        ...
        \[
            ...
            By Prop (\ref{proposition:16.2}) if $z_0$ is a pole of order $m$, then $\text{Res}(f, z_0) = \frac{\varphi^{(m-1)}(z_0)}{(m-1)!}$ where $f(z) = \frac{\varphi(z)}{(z-z_0)^m}$, $\varphi$ is analytic at a neighborhood of $z_0$ and $\varphi(z_0) \neq 0$. Thus, it is equivalent to:
            
            (\verifythis) $\text{Res}(f, z_0) = \frac{1}{(m-1)!} \lim_{z \to z_0} \left[ (z-z_0)^m f(z) \right]^{(m-1)}$
        \end{soln}

\begin{example}\label{example:21.3}
    Compute $\text{Res}(f, -2)$ for $f(z) = \frac{z+1}{(z-1)(z+2)^2}$.
\end{example}

\begin{soln}
 
  $\text{Res}(f, -2) = \lim_{z \to -2} \left[ (z+2)^2 f(z) \right]' = \lim_{z \to -2} \left( \frac{z+1}{z-1} \right)' = \lim_{z \to -2} \frac{(z-1) - (z+1)}{(z-1)^2} = \lim_{z \to -2} \frac{-2}{(z-1)^2} = -\frac{2}{9}$
\end{soln}

\begin{example}\label{example:21.4}    
    Compute $\text{Res}(f, 0)$ for $f(z) = \frac{1}{z(e^z - 1)}$.
\end{example}
\begin{soln}
    (\verifythis) $z=0$ is a pole of order 2.
    \[
...
\begin{example}\label{example:21.5}
...
$z=4$ is a pole of order 2 of $\frac{1}{(z-4)^2}$ (Justify!). Other than at $z=4$ the function $\frac{1}{(z-4)^2}$ is analytic. Thus by Prop (\ref{proposition:16.2}) for $\varphi(z) = 1$: $\text{Res} \left( \frac{1}{(z-4)^2}, 4 \right) = \varphi'(4) = 1' = 0$.
    
    By the Residue Theorem(\ref{theorem:residue.19.1}):
    \[
...
\begin{example}\label{example:21.6}
...
\begin{soln}
 ...
 $f(z) = \frac{1}{(z-1)(z-2)}$ has 2 simple poles $z=1, z=2$ (\verifythis !)
...
\begin{example}\label{example:21.7} Compute $\int_{|z|=3} \frac{z+1}{(z-1)(z+2)^2} dz$
\end{example}

\begin{soln}

     In the disc $\{z \in \mathbb{C}: |z| \le 3\}$ the function $f(z) = \frac{z+1}{(z-1)(z+2)^2}$ has 2 isolated singularities - poles at $z=1$ and at $z=-2$ (\verifythis). Therefore, by the Residue Theorem (\ref{theorem:residue.19.1})
\[
\int_{|z|=3} f(z) dz = 2\pi i \left[ \text{Res}(f, 1) + \text{Res}(f, -2) \right]
\]

$z=-2$: By Prop (\ref{proposition:16.2}) $f(z) = \frac{\varphi(z)}{(z+2)^2}$ where $\varphi(z) = \frac{z+1}{z-1}$ is analytic at a neighborhood of $z=-2$ and at $z=-2$, and $\varphi(-2) = \frac{1}{3} \neq 0$ (for $m=2$).
\[
\text{Res}(f, -2) = \varphi'(-2) = \left. \frac{(z-1) - (z+1)}{(z-1)^2} \right|_{z=-2} = -\frac{2}{9}
\]

$z=1$: By Prop (\ref{proposition:16.2}) $f(z) = \frac{\varphi(z)}{z-1}$ where $\varphi(z) = \frac{z+1}{(z+2)^2}$ is analytic at a neighborhood of $z=1$ and at $z=1$, and $\varphi(1) = \frac{2}{9} \neq 0$.
\[
...
\section{Implications of Cuachy's Theorems}

Now we state (and prove some of) a collection of surprising and powerful consequences of Cauchy's Theorem.

\subsection{Gauss's Mean Value Theorem}

\begin{theorem}[Gauss's Mean Value Theorem]\label{theorem:GaussMVT.20.1}
...
\begin{proof}
     ...
     Parametrize the contour $C$ by: $\gamma(\theta) = z_0 + Re^{i\theta}$, $0 \le \theta \le 2\pi$. By the CIF(\ref{theorem:Cauchy.Integral.formula.18.7}) and the definition of the contour integral (Def 17.7(\ref{definition:17.7}))
        \[
...
\begin{example}\label{example:22.2}
    Obtain an estimate on the absolute value of the $n$-th derivative of an analytic function $f$ in the disc $|z-z_0| \le R$.
    
    \begin{soln}
    
    By the Maximum Modulus Principle(\ref{theorem:Max.Modulus.Principle.20.2}) $\max |f(z)|$ is attained on the circle $|z-z_0| = R$ (depends only on $R$). Denote $\max |f(z)| = MR$. To estimate $|f^{(n)}(z_0)|$ we use Cor 18.6(\ref{corollary:18.6}) (extended CIF) and Prop 17.5(\ref{proposition:17.5}) (ML inequality):
        \[
