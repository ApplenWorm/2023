...
\begin{soln}
 ...
 \emphi{Note:} $f(z) = \frac{z}{(9-z^2)(z+i)}$ has simple poles at $z = -i, \pm 3$ (\verifythis) and only $z = -i$ is enclosed by $C$. Therefore:
	...
	By CIF~\ref{theorem:Cauchy.Integral.formula.18.7}:
    %%%%%%
...
\begin{corollary}[Extended version of the CIF]\label{corollary:18.9}
    ...
    \begin{soln}
      Set $f(z) = z^3$, apply Cor~\ref{corollary:18.9} with $n=3$:
        \[
...
Recall (Def~\ref{definition:16.2}): The coefficient $b_1$ of the term $\frac{1}{z-z_0}$ is called the residue of the singularity at $z=z_0$, denoted by $\text{Res}(f, z_0)$. We have seen the definition and examples on week 8!
...
Draw small positively oriented circles $C_k$ around the singularities $z_k$ such that they all inside $C$ and they are disjoint - choose radius small enough, so it is possible.
    ...
    By extended Deformation Principle (Cor~\ref{corollary:18.6}):
    \[
        ...
        If $z_k$ is a simple pole, then by Prop (\ref{proposition:16.2}) ($m=1$) $f(z) = \frac{\varphi(z)}{z-z_k}$ where $\varphi(z)$ is analytic at $z_k$, therefore by CIF~\ref{theorem:Cauchy.Integral.formula.18.7}
        \[
...
\begin{proof}
     ...
     Parametrize the contour $C$ by: $\gamma(\theta) = z_0 + Re^{i\theta}$, $0 \le \theta \le 2\pi$. By the CIF~\ref{theorem:Cauchy.Integral.formula.18.7} and the definition of the contour integral (Def~\ref{definition:17.7})
        \[
...
\begin{example}\label{example:22.2}
    ...
    By the Maximum Modulus Principle(\ref{theorem:Max.Modulus.Principle.20.2}) $\max |f(z)|$ is attained on the circle $|z-z_0| = R$ (depends only on $R$). Denote $\max |f(z)| = M_R$. To estimate $|f^{(n)}(z_0)|$ we use Cor~\ref{corollary:18.6} (extended CIF) and Prop~\ref{proposition:17.5} (ML inequality):
        \[
