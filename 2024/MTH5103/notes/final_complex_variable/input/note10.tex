...
\begin{example}\label{example:22.2}
    ...
    By the Maximum Modulus Principle(\ref{theorem:Max.Modulus.Principle.20.2}) $\max |f(z)|$ is attained on the circle $|z-z_0| = R$ (depends only on $R$). Denote $\max |f(z)| = MR$. To estimate $|f^{(n)}(z_0)|$ we use Cor 18.6 (extended CIF) and Prop 17.5 (ML inequality):
        \[
