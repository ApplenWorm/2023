...
Take a circle centered at $z_0$ of radius $R$. Then, by Cauchy's Estimate (Prop 20.3(\ref{proposition:20.3})): $|f'(z_0)| \le \frac{M}{R}$ and this inequality holds for all choices of $R > 0$. 
...
By Prop 16.2 (\ref{proposition:16.2}) for $\varphi(z) = \frac{z}{(z - (-5 - \sqrt{24}))^2}$ and $m = 2$
\[
...
The length of $C_R^+$ is $\pi R$, thus by ML inequality (Prop 17.5 (\ref{proposition:17.5}))
\[
...
$z_1$ (\verifythis): Since $z_1$ is a simple pole we can use Cor 16.5(\ref{corollary:16.5}) with $p(z) = 1$, $q(z) = z^4 + 16$, $q'(z) = 4z^3$ and obtain:
\[
...
$z_2$: Again, using Cor 16.5(\ref{corollary:16.5}):
\[
...
\begin{proof}
    ...
    \[
        ...
        \[
            ...
            we conclude (by Prop 16.2 (\ref{proposition:16.2})) that $z=0$ is a pole of order 3.
        \end{proof}
