...
\graphicspath{{./input/graphs/note11/}}
...
\subsection{Cauchy's Estimate}


We continue with consequences of Cauchy's Theorem. The first one is the Liouville Theorem: suppose $f$ is entire (analytic on $\mathbb{C}$). By the Maximum Modulus Principle as $|z|$ gets larger, 
we can find points with $|f(z)|$ larger and larger, since the complex plane is unbounded. Does this mean that $|f(z)|$ must be unbounded for $z \in \mathbb{C}$? 
To address this question we need to investigate the size of $|f^{(n)}(z)|$.
\begin{proposition}[Cauchy's Estimate]\label{proposition:20.3}
     Let $f$ be analytic on and inside a circle $C$ centered at $z_0$ of radius $R$. Let $M = \max |f(z)|$, $z \in C$. Then, for all $n \ge 0$,
    \[
        |f^{(n)}(z_0)| \le \frac{n! M}{R^n}
        \]
    \end{proposition}
\begin{proof}
From the extended Cauchy Integral Formula (Cor~\ref{corollary:18.9}):
\[
        f^{(n)}(z_0) = \frac{n!}{2\pi i} \int_C \frac{f(z)}{(z-z_0)^{n+1}} dz
  \]
        
        Thus, by ML inequality (Prop~\ref{proposition:17.5})
        \[
...
Take a circle centered at $z_0$ of radius $R$. Then, by Cauchy's Estimate (Prop~\ref{proposition:20.3}): $|f'(z_0)| \le \frac{M}{R}$ and this inequality holds for all choices of $R > 0$. 
...
\section{Applications of Contour Integration to Real Integrals}

\subsection{Trigonometric integrals via residues}
...
\end{example}
\begin{soln}
    Let us make substitutions as in (*) and get:
    \[
...
By Cor~\ref{corollary:16.5}, since $q'(\frac{1}{2}) = -\frac{3}{4}$,
\[
\text{Res} \left( \frac{z^6 + 1}{z^3 (2z - 1) (z - 2)}, \frac{1}{2} \right) = \frac{p(\frac{1}{2})}{q'(\frac{1}{2})} = \frac{\frac{65}{64}}{-\frac{3}{4}} = -\frac{65}{48}
\]

$z_0 = 0$: By Prop~\ref{proposition:16.2} for $\varphi(z) = \frac{z^6 + 1}{2z^2 - 5z + 2}$: $\text{Res} (f, 0) = \frac{\varphi''(0)}{2!} = \frac{21}{8}$
\[
...
\end{example}
    \begin{soln}
         Since the integration is on $[0, \pi]$ we cannot use directly the formula. The function $\frac{1}{(2 + \cos^2 t)^2}$ is even, thus
        \[
...
The function $f(z) = \frac{z}{(z^2 + 10z + 1)^2}$ has 2 poles: $z_{1,2} = -5 \pm \sqrt{24}$, 
both of order 2 (\verifythis), only $z_1 = -5 + \sqrt{24}$ is inside $|z| \le 1$ (\verifythis). 
By Prop~\ref{proposition:16.2} for $\varphi(z) = \frac{z}{(z - (-5 - \sqrt{24}))^2}$ and $m = 2$
\[
...
\begin{exercise}
    EXERCISE:
    \begin{enumerate}
        \item Prove that $\int_0^{2\pi} \frac{\sin^2 t}{5 - 4 \cos t} dt = \frac{\pi}{4}$
        \item Compute $\int_0^{\frac{\pi}{2}} \frac{dt}{(4 + \sin^2 t)^2}$
    \end{enumerate}
\end{exercise}
%(182)
...
\subsection{Sum of Infinite Rational Functions}
...
The length of $C_R^+$ is $\pi R$, thus by ML inequality (Prop~\ref{proposition:17.5})
\[
...
\]
Note that we could conclude the result directly from Thm~\ref{theorem:21.1}.
\end{soln}
...
\begin{remark}\label{remark:20.37}
    ...
    \[\int_0^{\infty} f(x) dx = \frac{1}{2} \int_{-\infty}^{\infty} f(x) dx
        \]
        and we can use Thm~\ref{theorem:21.1} again.
    \end{remark}

Let us see one more example.
\begin{example}\label{example:24.2}
...
To use Thm~\ref{theorem:21.1} we need to compute $\text{Res} \left( \frac{1}{z^4 + 16}, z_1 \right)$ and $\text{Res} \left( \frac{1}{z^4 + 16}, z_2 \right)$.

$z_1$ (\verifythis): Since $z_1$ is a simple pole we can use Cor~\ref{corollary:16.5} with $p(z) = 1$, $q(z) = z^4 + 16$, $q'(z) = 4z^3$ and obtain:
\[
...
$z_2$: Again, using Cor~\ref{corollary:16.5}:
\[
\text{Res} \left( \frac{1}{z^4 + 16}, z_2 \right) = \frac{1-i}{32\sqrt{2}}
\]

$\Rightarrow$ by Thm~\ref{theorem:21.1} (and \ref{remark:20.37}) we obtain
\[
...
\begin{example}\label{example:24.3}
...
\begin{proof}
    ...
    \[
        ...
        \[
            ...
            we conclude (by Prop~\ref{proposition:16.2}) that $z=0$ is a pole of order 3.
        \end{proof}
