...
\begin{proposition}[Cauchy's Estimate]
    ...
    \end{proposition}
\begin{proof}
From the extended Cauchy Integral Formula (Cor 18.9):
\[
...
\begin{theorem}[Liouville's Thm]
    Thm 20.4 If $f$ is entire and bounded, then $f$ is a constant function.
\end{theorem}
\begin{proof}
...
\begin{theorem}[Fundamental Theoreom of Algebra]
Thm 20.5  Any nonconstant complex polynomial $p(z) = a_0 + a_1 z + \dots + a_n z^n$ ($n \ge 1$, $a_j \in \mathbb{C}$, $a_n \neq 0$) has at least one root in $\mathbb{C}$.
\end{theorem}
\begin{proof}
...
\begin{theorem}\label{theorem:21.1}
    ...
    Thm 21.1: Let $m \ge n+2$, $f(x) = \frac{P_n(x)}{Q_m(x)}$ rational function, such that the integral (1) converges. Assume that $z_1, z_2, \dots, z_K$ are poles of $f(z)$, 
...
\begin{remark}\label{remark:20.37}
    ...
    \[\int_0^{\infty} f(x) dx = \frac{1}{2} \int_{-\infty}^{\infty} f(x) dx
        \]
        and we can use Thm 21.1 (\ref{theorem:21.1}) again.
    \end{remark}
