...
\begin{example} 17.1:} Compute: 1) $\text{Ln } i$ 2) $\text{Ln } (-4)$
...
\end{example}
\begin{example} 17.2:} Find an analytic function $f(z)$ such that $\text{Re } f(z) = \arctan \frac{y}{x}$ and $f(-i) = \frac{3\pi}{2} + 2i$.
...
\end{enumerate}
\begin{proof}
By formula (*), Prop 1.2, and Prop 1.3, we get:
...
\begin{example} 17.3:} Study the function $w = \text{Ln}(z^2 + 4)$.
...
\begin{example} 17.4:} Compute: 1) $\arcsin 2$ 2) $\arcsin i$.
...
\begin{example} 18.1:} Let $f(z) = \frac{1}{(z-1)(z-2)}$. Then $f$ has isolated singularities at $z = 1$ and $z = 2$ \{f is a rational function $\implies$ analytic everywhere except at the zeros of the denominator\}.
...
\begin{example} 18.3:} Classify the singularities of $f(z) = e^{\frac{1}{z}}$ and compute the residue of $f$ at $z = 0$.

Sol: Since $f(z)$ is analytic everywhere except for $z = 0$, $f$ has an isolated singularity at $z = 0$. To determine the nature of this singularity, we write the Laurent series expansion: 
