...
\textbf{Prop 15.2:} Every branch of the function $\text{Ln } z$ is an analytic function in the cut-plane (cutted along some ray starting at the origin) and $(\text{Ln } z)' = \frac{1}{z}$.
...
\textbf{Prop 16.1:} $z_0$ is a removable singularity of $f(z)$ if and only if $\lim_{z \to z_0} (z - z_0)f(z) = 0$.
...
\textbf{Prop 16.2:} $z = z_0$ is a pole of order $m > 0$ of $f \iff f$ can be expressed as $f(z) = \frac{\varphi(z)}{(z - z_0)^m}$ where $\varphi(z)$ is analytic at a neighborhood of $z_0$ (and at $z_0$) and $\varphi(z_0) \neq 0$. Moreover, $\text{Res}(f, z_0) = \frac{\varphi^{(m-1)}(z_0)}{(m-1)!}$.
