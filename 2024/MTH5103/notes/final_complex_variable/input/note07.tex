...
By Def 15.1(\ref{definition:15.1}): $e^{u+iv} = r(\cos \varphi + i\sin \varphi) \implies e^u (\cos v + i\sin v) = r(\cos \varphi + i\sin \varphi)$.
...
\begin{example}\label{example:17.1} Compute: 1) $\text{Ln } i$ 2) $\text{Ln } (-4)$

    \begin{soln}
                        1) $\text{Ln } i = \ln |i| + i(\arg i + 2\pi k) = \ln 1 + i(\frac{\pi}{2} + 2\pi k) = i(\frac{\pi}{2} + 2\pi k)$.
...
\begin{soln}
    ...
    In Ex 11.3(\ref{example:11.3}) we have seen that $f(z) = \arctan \frac{y}{x} - \frac{i}{2} \ln(x^2 + y^2) + ic$.
...
\begin{soln}
    ...
    $w = \text{Ln}(z^2 + 4) = \text{Ln}(z + 2i)(z - 2i) = \text{Ln}(z + 2i) + \text{Ln}(z - 2i)$ (Prop 15.1 1)) $\implies w$ has 2 branch points $z = 2i$ and $z = -2i$. Therefore, $w$ is analytic in the cut-plane where we cut along 2 rays that start at $z = \pm 2i$ (including $\pm 2i$):
...
\begin{definition}\label{definition:15.3}
    ...
    1) $\arcsin z$ is $w \in \mathbb{C}$ such that $\sin w = z$.
...
\begin{example}\label{example:17.4} 
    Compute: 1) $\arcsin 2$ 2) $\arcsin i$.
\begin{soln}
1) $\arcsin 2 = -i\text{Ln}(2i \pm \sqrt{1-4}) = -i\text{Ln}((2 \pm \sqrt{3})i) = -i(\ln(2 \pm \sqrt{3}) + i(\frac{\pi}{2} + 2\pi k)) = \frac{\pi}{2} + 2\pi k - i\ln(2 \pm \sqrt{3})$. If, for example, $k = 1$, $\arcsin 2$ attains two values: $\arcsin 2 \approx \frac{5}{2}\pi \pm i \cdot 1.32$.
...
By Thm 13.2(\ref{theorem:13.2}) (Laurent series) Such $f$ has a Laurent series expansion valid on $D'$:
...
Now we can see that there are 3 options for the principal part in (*): there is no principal part; there is principal part and it has finitely many terms; or there is an infinite principal part. 
...
\begin{proof}
 ...
 neighborhood of $z = 2$ (including $z = 2$) and $\varphi(2) = 1 \neq 0$, thus, by Prop 16.2 (\ref{proposition:16.2}) $z = 2$ is a simple pole.

$z = 1$: In the same way: $f(z) = \frac{\tilde{\varphi}(z)}{z-1}$, $\tilde{\varphi}(z) = \frac{1}{z-2}$, $\tilde{\varphi}$ is analytic in a 
neighborhood of $z = 1$ (including $z = 1$), and $\tilde{\varphi}(1) = -1 \neq 0$. Thus, by Prop 16.2(\ref{proposition:16.2}) $z = 1$ is a simple pole.
\end{proof}
By Prop 16.2(\ref{proposition:16.2}): $\text{Res}(f, 2) = \frac{\varphi(2)}{0!} = 1; \text{Res}(f, 1) = \frac{\tilde{\varphi}(1)}{0!} = -1$.
...
\begin{soln}
    1) Since $\lim_{z \to 3} \frac{(z-3)(z+3)}{z-3} = 6$ and $\lim_{z \to 3} (z-3) \cdot \frac{(z-3)(z+3)}{z-3} = 0$ by Prop 16.1 (\ref{proposition:16.1})$z = 3$ is a removable singularity. 
    ...
    i) The Laurent series for $f$ in $0 < |z| < R$ is $f(z) = \frac{1}{z}(z - \frac{z^3}{3!} + \frac{z^5}{5!} - \dots) = 1 - \frac{z^2}{3!} + \frac{z^4}{5!} - \dots$ $\implies$ the 
    principal part does not exist $\implies z = 0$ is a removable singularity.
    
    ii) $\lim_{z \to 0} z \cdot \frac{\sin z}{z} = \lim_{z \to 0} \sin z = 0 \implies$ by Prop 16.1 (\remref{proposition:16.1})$z = 0$ is a removable singularity.
\end{soln}
