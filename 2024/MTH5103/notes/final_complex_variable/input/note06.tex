...
\begin{example} 15.1:} Determine the convergence or divergence of:
...
\begin{example} 15.2:} For what values of $z$ does the power series $\sum_{n=0}^{\infty} \left(\frac{4+i}{6}\right)^n (z-3)^n$ converge?
...
\begin{example} 15.3:} Find the power series expansion of the function $f(z) = \frac{1}{2z-3}$ about the point $z_0 = 0$ and determine the radius of convergence.
...
\begin{example} 15.6:} We have seen: $e^z = \sum_{n=0}^{\infty} \frac{z^n}{n!}$ converges absolutely for all $z \in \mathbb{C} \implies$ the radius of convergence $R = \infty \implies e^z$ is
...
\begin{example} 15.7:} Let $f(z) = \frac{1}{2z-3}$. What is the power series of $f$ about $z_0 = 2$?
...
\begin{example} 15.8:} let $f(z) = \frac{1}{1-z}$. We have a power series for $|z| < 1$: $f(z) = \sum_{n=0}^{\infty} z^n$ (geometric series). What can we do for $|z| > 1$?
...
\begin{example} 15.9:} By definition: $e^{z + \frac{1}{z}} = \sum_{n=0}^{\infty} \frac{1}{n!} z^n$ on $A = \{z \in \mathbb{C}: 0 < |z| < \infty\}$.
...
\begin{example} 15.10:} What is the laurent series of $f(z) = \frac{1}{(z-1)(z-2)}$ on the annulus $A = \{z \in \mathbb{C}: 1 < |z| < 2\}$?
...
\begin{example} 15.11:} Obtain the series expansion for $f(z) = \frac{1}{z^2 + 4}$ valid in the region $|z - 2i| > 4$.
...
\begin{example} 16.1:} For $w = z + 1$: the whole domain is moved by 1 to the right parallel to x-axis. For $w = z - 2i$ - the whole domain is moved by 2 down parallel to y-axis. For $w = z - 1 + 2i$ - the whole domain is moved to the direction of the vector $-1 + 2i$ and by distance $\sqrt{5}$.
\end{example}
...
\begin{example} 16.2.} Points on the circle $|z| = 2$ under $w = 3z$ pass to
...
\end{proposition}
\begin{example} 16.3:} Find the Möbius transformation that sends $z_1 = 1 \to -i = w_1, z_2 = 0 \to -1 = w_2, z_3 = i \to 1 = w_3$.
...
\begin{example} 16.4:} Find the image of the upper-half circle $D = \{z \in \mathbb{C}: |z| < 1, \text{Im } z > 0\}$ under the Möbius transformation $w = \frac{i-iz}{1+z}$.
...
\begin{example} 16.5:} Find the Möbius transformation sending $-1 \to i, 0 \to 1, i \to -i$.
